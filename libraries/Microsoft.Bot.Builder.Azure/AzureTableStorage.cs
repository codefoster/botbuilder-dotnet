...
namespace Microsoft.Bot.Builder.Azure
{
    ...
    public class AzureTableStorage : IStorage
    {
        ...
        public async Task Delete(string[] keys)
        {
            ...
            foreach (var key in keys.Select(k => new EntityKey(k)))
            {
        ...
        public async Task<StoreItems> Read(string[] keys)
        {
            ...
            {
                var entityKey = new EntityKey(key);
                var tableEntity = await Table.ExecuteAsync(TableOperation.Retrieve<DynamicTableEntity>(entityKey.PartitionKey, entityKey.RowKey));
                if (tableEntity.HttpStatusCode == (int)HttpStatusCode.OK)
                {
                    ...
                    storeItems[key] = storeItem.Entity;
                }
            }
        ...
        public async Task Write(StoreItems changes)
        {
            ...
            {
                var storeEntity = new StoreItemEntity(new EntityKey(change.Key), change.Value);
                ...
                if (storeEntity.ETag == null || storeEntity.ETag == "*")
                {
                    ...
                    tasks.Add(Table.ExecuteAsync(TableOperation.InsertOrReplace(tableEntity)));
                }
                else if (storeEntity.ETag.Length > 0)
                {
                    // Optimistic Update
                    tasks.Add(Table.ExecuteAsync(TableOperation.Replace(tableEntity)));
                }
                else
                {
        ...
        private class StoreItemEntity
        {
            ...
            public static StoreItemEntity AsStoreItemEntity(TableResult tableEntity)
            {
                ...
                if (value is StoreItem storeItem)
                {
            ...
            private static dynamic ApplyProperties(Dictionary<string, EntityProperty> properties, dynamic item)
            {
